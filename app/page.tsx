...
export default function Home() {
  ...
  return <main className="shell">
    <Sidebar user={user} active={active} allowedMenu={allowedMenu} setActive={setActive} logout={logout}/>
...
function Sidebar({ user, active, allowedMenu, setActive, logout }: { user: User; active: string; allowedMenu: typeof menu; setActive: (value: string) => void; logout: () => void }) {
  return <aside className="sidebar"><div className="brand"><span className="brandMark">SIC</span><div><b>SIC</b><small>Universal Nicaragua</small></div></div><nav aria-label="Navegación principal">{menuGroups.map(group=>{const items=allowedMenu.filter(item=>group.items.includes(item.nombre));return items.length?<section className="navSection" key={group.label}><p className="navLabel">{group.label}</p>{items.map(item=><button key={item.nombre} className={active===item.nombre?"navItem active":"navItem"} onClick={()=>setActive(item.nombre)}><MenuIcon name={item.icono}/><span>{item.nombre}</span></button>)}</section>:null;})}</nav><div className="sidebarFoot"><span className="avatar">{user.nombre.split(" ").map(word=>word[0]).slice(0,2).join("")}</span><div><b>{user.nombre}</b><small>{nombresRol[user.rol]}</small></div><button aria-label="Cerrar sesión" onClick={logout}>Salir</button></div></aside>;
}
