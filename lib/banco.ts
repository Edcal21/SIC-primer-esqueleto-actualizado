...
export function valorFecha(value: unknown): string | null {
  if (value instanceof Date && !Number.isNaN(value.getTime())) {
    // XLS puede representar la medianoche unos segundos antes por su precisión histórica.
    const diaUtc = 24 * 60 * 60 * 1000;
    return new Date(Math.round(value.getTime() / diaUtc) * diaUtc).toISOString().slice(0, 10);
  }
  const raw = valorTexto(value);
  if (!raw) return null;
